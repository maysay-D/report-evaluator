...
try:
    ...
    raise SystemExit("GUIにはTk対応のPythonが必要です。READMEのGUIセットアップを確認してください。\n" + str(exc)) from exc
...
class ReportEvaluatorApp:
    def __init__(self, root):
        ...
        self.metrics = None
        self.snapshot = None
        self.status = tk.StringVar(value="レポートを貼り付けるか、ファイルを開いてください。")
        self.system = tk.StringVar(value="baseline")
        self.actions = []
        style = ttk.Style(root)
        style.theme_use("clam")
        style.configure("TFrame", background="#f4f6fa")
        style.configure("TLabel", background="#f4f6fa", foreground="#243047", font=("", 11))
        ...
        ttk.Label(shell, text="レポートを、伝わる文章へ。", style="Title.TLabel").pack(anchor="w")
        ttk.Label(shell, text="冗長性・論理整合性・要求網羅度を確認し、推敲に役立てます。").pack(anchor="w", pady=(5, 12))
        bar = ttk.Frame(shell)
        bar.pack(fill="x", pady=(0, 10))
        ttk.Label(bar, text="評価方式").pack(side="left")
        self.selector = ttk.Combobox(bar, textvariable=self.system, values=("baseline", "improved"), state="readonly", width=12)
        self.selector.pack(side="left", padx=8)
        ttk.Label(bar, text="baseline: メモの手法 / improved: 追加モデルが必要").pack(side="left")
        ...
        for frame, title in [(self.input_tab, "1  入力"), (self.output_tab, "2  評価結果"), (self.metrics_tab, "3  システム評価")]:
            self.tabs.add(frame, text=title)
        self.build_input()
        self.build_results()
        self.build_metrics()
        ttk.Label(shell, textvariable=self.status, wraplength=1000).pack(anchor="w", pady=(10, 0))
    ...
    def text_box(self, parent, height=5, readonly=False):
        widget = ScrolledText(parent, height=height, wrap="word", font=("", 12),
                              padx=12, pady=10, relief="flat", borderwidth=1, undo=not readonly)
        if readonly:
    ...
    def build_input(self):
        ...
        ttk.Label(self.input_tab, text="問題文（baselineでは採点に直接使用しません）").pack(anchor="w")
        ...
        ttk.Label(words, text="必須キーワード（カンマ・読点・改行で区切る）").grid(row=0, column=0, sticky="w")
        ttk.Label(words, text="推奨キーワード（任意）").grid(row=0, column=1, sticky="w")
    ...
    def build_results(self):
        self.score = tk.StringVar(value="まだ評価結果がありません。入力画面で「評価する」を押してください。")
        ttk.Label(self.output_tab, textvariable=self.score, style="Score.TLabel", wraplength=980).pack(anchor="w", pady=(0, 8))
        ttk.Label(self.output_tab, text="点数は高いほど良好。自動検出は候補であり、文章の正しさを保証しません。結果は評価時の本文です。", wraplength=980).pack(anchor="w")
        bar = ttk.Frame(self.output_tab)
        bar.pack(fill="x", pady=8)
        self.save_result = self.button(bar, "入力・結果をJSONで保存", lambda: self.save(self.result, "report_result.json"))
        notebook = ttk.Notebook(self.output_tab)
        notebook.pack(fill="both", expand=True)
        self.views = {}
        for key, label, legend in [
            ("redundancy", "冗長性", "黄色マーカー：似た文の繰り返し。新しい情報がなければ文を統合・削除しましょう。"),
            ("contradiction", "論理整合性", "赤い下線：矛盾候補。前後の文の主語・条件・否定表現を確認しましょう。"),
            ("keyword", "要求網羅度", "緑色マーカー：一致したキーワード。未検出の必須語は説明や根拠を補いましょう。"),
        ]:
    ...
    def build_metrics(self):
        ttk.Label(self.metrics_tab, text="正解データと比較して、評価システムの性能を確認", style="Score.TLabel").pack(anchor="w")
        ttk.Label(self.metrics_tab, text="人手の正解ラベルを含むJSON / JSONLを選択してください。単一レポートの点数とは別の指標です。", wraplength=980).pack(anchor="w", pady=8)
        bar = ttk.Frame(self.metrics_tab)
        bar.pack(fill="x", pady=(0, 10))
        self.button(bar, "正解データを選んで評価", self.evaluate_dataset)
        self.save_metrics = self.button(bar, "指標・誤り分析をJSONで保存", lambda: self.save(self.metrics, "system_evaluation.json"))
    ...
    def load_sample(self):
        try:
            req = EvaluationRequest.model_validate_json((Path(__file__).parent / "examples/input.json").read_text(encoding="utf-8"))
            for widget, value in [(self.report, req.report.text), (self.prompt, req.assignment.prompt),
    ...
    def start_job(self, job, callback):
        ...
        self.status.set("評価中です。improvedの初回はモデルの読み込みに時間がかかります。")
        def work():
            try:
    ...
    def poll(self):
        try:
            ...
            if error is not None:
                self.status.set("評価に失敗しました。入力や依存関係を確認して再実行してください。")
                messagebox.showerror("評価に失敗しました", error + "\n\nimprovedには uv sync --extra improved と初回のモデル取得が必要です。")
            else:
                callback(value)
                self.status.set("評価が完了しました。入力を編集した場合は、もう一度評価してください。")
    ...
    def evaluate(self):
        try:
            req = make_request(*(w.get("1.0", "end-1c") for w in (self.report, self.prompt, self.required, self.recommended)))
        except ValueError as exc:
            messagebox.showerror("入力を確認してください", str(exc))
            return
        system = self.system.get()
        self.start_job(lambda: (req, build_evaluator(system).evaluate(req)), self.show_results)

    def show_results(self, value):
        req, result = value
        self.snapshot = req
        self.result = {"request": req.model_dump(), "result": result.model_dump()}
        coverage = f"{result.coverage_score:.1f}" if result.keywords else "未評価（キーワード未指定）"
        self.score.set(f"{result.system}  |  総合 {result.overall_score_100:.1f}/100  |  冗長性 {result.redundancy_score:.1f}  |  論理整合性 {result.consistency_score:.1f}  |  網羅度 {coverage}")
        for key, (view, detail) in self.views.items():
            ...
            for h in highlights:
                ...
                start = self.root.tk.call("string", "length", req.report.text[:h.start])
                end = self.root.tk.call("string", "length", req.report.text[:h.end])
                view.tag_add(key, f"1.0 + {start} chars", f"1.0 + {end} chars")
            lines = []
            if key == "keyword":
                for k in result.keywords:
                    lines.append(f"{'必須' if k.required else '推奨'} / {k.keyword} : {'検出（' + k.match_type + '）' if k.matched else '未検出 → 本文に説明を追加してください'}")
                if not result.keywords:
                    lines.append("キーワード未指定のため網羅度は未評価です。既存処理では100点として総合点に含まれます。")
                lines.append("キーワードの出現は、問題文への十分な回答を意味しません。baselineは空白除去・大小文字統一後に照合します。表記によってはマーカーが付かない場合があります。")
            else:
                for h in highlights:
                    lines.append(f"文{h.sentence_index + 1 if h.sentence_index is not None else '?'}：{h.reason}\n「{req.report.text[h.start:h.end]}」")
                if not highlights:
                    lines.append("該当する指摘箇所は検出されませんでした。")
                if key == "redundancy":
                    d = result.diagnostics.get("redundancy", {})
                    lines.append(f"bigram反復率: {d.get('bigram_repeat_ratio', '—')} / trigram反復率: {d.get('trigram_repeat_ratio', '—')}")
                    lines.append("反復率は本文全体の指標、マーカーは類似文の指摘です。重複文は要点を残して統合してください。")
                else:
                    lines.append("同じ対象について肯定と否定が混在していないか確認し、条件や例外がある場合は明記してください。")
            self.replace(detail, "\n\n".join(lines))
    ...
    def save(self, payload, filename):
        if payload is None:
            return
        path = filedialog.asksaveasfilename(initialfile=filename, defaultextension=".json", filetypes=[("JSON", "*.json")])
        if path:
            try:
...
def main():
    try:
        ...
        raise SystemExit("GUIの起動にはデスクトップ環境とTkが必要です。\n" + str(exc)) from exc
